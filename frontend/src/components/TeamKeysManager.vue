...
async function load() {
  ...
  error.value = "";
  try {
    keys.value = await apiCall<ApiKeyDetail[]>(`/teams/${props.teamId}/api-keys`);
  } catch (err) {
    error.value = err instanceof ApiError ? err.message : "이 팀의 팀장만 팀 관리 키 목록을 볼 수 있습니다";
  } finally {
    loading.value = false;
  }
}
...
<template>
  <div class="manager">
    <p class="hint">이 팀 소속 모든 프로젝트에 접근하는 관리 키 - 팀장만 만들고 볼 수 있다.</p>
