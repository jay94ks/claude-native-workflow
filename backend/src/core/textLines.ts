...
import { compilePosixEre } from "./posixRegex.js";
...
/** POSIX ERE(정식 규격 - `#posix-regex-parser`, `./posixRegex.ts` 참고)
 * 패턴으로 콘텐츠를 줄 단위 검색한다 - Grep 도구의 "content" 출력
 * 모드와 같은 모양(줄 번호+텍스트). 패턴이 실제로 잘못됐으면(대괄호
 * 미종료, 알 수 없는 POSIX 클래스 등) `PosixRegexError`를 던진다 -
 * 조용한 리터럴 폴백 없음. */
export function grepLines(content: string, pattern: string, opts: GrepOptions = {}): GrepMatch[] {
  const re = compilePosixEre(pattern, !!opts.caseInsensitive);
