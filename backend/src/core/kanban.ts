...
// ---------------------------------------------------------------- 카드 코멘트 (설계자간 채널 - CLI/MCP 없음)

export interface KanbanCardCommentDetail {
  id: string;
  cardId: string;
  body: string;
  authorId: string;
...
export async function addKanbanCardComment(
  ...
  body: string,
  authorId: string,
): Promise<KanbanCardCommentDetail> {
  const db = getDb();
  const card = await db.kanbanCard.findUnique({ where: { trackingCode } });
  if (!card) throw new Error(`카드를 찾을 수 없습니다: ${trackingCode}`);
  const row = await db.kanbanCardComment.create({ data: { cardId: card.id, body, authorId } });
  await realtimePublish(projectChangesTopic(card.projectId), {
    entity: "kanbanCardComment",
    action: "create",
    id: row.id,
    trackingCode,
    at: new Date().toISOString(),
...
export async function listKanbanCardComments(trackingCode: string): Promise<KanbanCardCommentDetail[]> {
  const db = getDb();
  const card = await db.kanbanCard.findUnique({ where: { trackingCode } });
  if (!card) throw new Error(`카드를 찾을 수 없습니다: ${trackingCode}`);
  return db.kanbanCardComment.findMany({ where: { cardId: card.id }, orderBy: { createdAt: "asc" } });
}

export async function editKanbanCardComment(
  commentId: string,
  body: string,
  requesterId: string,
): Promise<KanbanCardCommentDetail> {
  const db = getDb();
  const existing = await db.kanbanCardComment.findUnique({ where: { id: commentId } });
  if (!existing) throw new Error(`코멘트를 찾을 수 없습니다: ${commentId}`);
  if (existing.authorId !== requesterId) throw new Error("본인이 작성한 코멘트만 수정할 수 있습니다");
  const row = await db.kanbanCardComment.update({ where: { id: commentId }, data: { body } });
  const card = await db.kanbanCard.findUnique({ where: { id: existing.cardId } });
  if (card) {
    await realtimePublish(projectChangesTopic(card.projectId), {
      entity: "kanbanCardComment",
      action: "update",
...
export async function deleteKanbanCardComment(commentId: string, requesterId: string): Promise<void> {
  const db = getDb();
  const existing = await db.kanbanCardComment.findUnique({ where: { id: commentId } });
  if (!existing) throw new Error(`코멘트를 찾을 수 없습니다: ${commentId}`);
  if (existing.authorId !== requesterId) throw new Error("본인이 작성한 코멘트만 삭제할 수 있습니다");
  const card = await db.kanbanCard.findUnique({ where: { id: existing.cardId } });
  await db.kanbanCardComment.delete({ where: { id: commentId } });
  if (card) {
    await realtimePublish(projectChangesTopic(card.projectId), {
      entity: "kanbanCardComment",
      action: "delete",
