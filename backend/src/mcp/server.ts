...
async function main() {
  ...
  // 서로를 구분하고(X-Session-Id 헤더는 이 MCP 서버가 공유하는
  // apiFetch()가 이미 자동으로 붙임, 이 도구들엔 그 인자가 없음)
  // "지금 뭘 작업 중인지"를 광고판처럼 등록해 다른 세션에 알린다 -
  // 락이 아니라 경고용(WorkClaim이 있어도 저장/전이는 그대로 진행되고
  // 관련 mutation 응답의 notices에 경고만 뜬다).
