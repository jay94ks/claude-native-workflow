...
1. `#cli-mcp-audit-script` - 이후 모든 라운드의 QA 부담을 줄여주는
   인프라 투자라 상대적으로 먼저 볼 가치가 있음.
...
### `#login-rate-limit`
**로그인 시도 제한(rate limit) 없음** - 무차별 대입 방어가 없다.
...
### `#cli-mcp-audit-script`
**이 대칭성 감사를 정기적으로 반복할 공식 절차가 없다** - 지금은
"QA 패스" 때 수작업 스크립트(`mcp-test.mjs`류)로 확인했다. 새 기능
라운드가 끝날 때마다 자동으로 CLI/MCP 이름 목록을 비교해 차이를
보고하는 작은 스크립트(`npm run audit:cli-mcp` 같은)를 만들어두면,
사람이 매번 기억해서 감사할 필요가 없어진다 - 가벼운 투자 대비 효과가
큰 후보.
