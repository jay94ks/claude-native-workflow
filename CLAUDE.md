...
## 이 컴퓨터의 로컬 운영 설치(`C:\CNW`) - 이 저장소와 다른 존재

이 컴퓨터(`C:\GitHub\claude-native-workflow`가 있는 개발 머신)에는
GitHub origin에서 별도로 clone한 **실제 운영 설치**가
`C:\CNW`(`backend/docker`의 `.env`에 `COMPOSE_PROJECT_NAME=cnw`)에
있다 - 이 저장소는 claude-native-workflow **자신을 개발**하는 곳이고,
`C:\CNW`는 그 결과물을 **실제로 쓰는** 별개의 설치다. 둘을 혼동하면
안 되는 이유:

- **데이터가 진짜다**: `C:\CNW`의 프로젝트/문서/git 저장소는 QA용
  스크래치 데이터가 아니라 실제로 계속 쓰이는 데이터로 취급한다 -
  이 저장소에서 QA/리허설 중 만든 테스트 프로젝트를 정리하듯 가볍게
  지우거나 건드리지 않는다. `C:\CNW` 안에서 뭔가 만들고 지우는 등의
  작업이 필요하면 먼저 설계자에게 확인한다(이 저장소 자신의 QA용
  스크래치 프로젝트와는 위험도가 다르다).
- **Docker 스택이 완전히 분리돼 있다**: 이 저장소의 `backend/docker`
  스택과 `C:\CNW\backend\docker` 스택은 `COMPOSE_PROJECT_NAME`이 달라
  볼륨/네트워크/컨테이너가 전혀 안 겹친다(겹치면 안 되는 이유와 사고
  재현 내용은 DESIGN-NOTES.md "실제 신규 설치(C:\CNW) 구성 중 발견한
  신규 설치 버그 2건" 라운드 참고) - 이 저장소에서 `docker compose`
  명령을 실행할 때 `C:\CNW`에 영향이 없는지, 반대로 `C:\CNW`에서 작업할
  때 이 저장소의 dev 스택에 영향이 없는지 항상 프로젝트 이름을
  의식한다.
- **코드 변경은 이 저장소에서, 실제 반영은 `git pull`로만**: `C:\CNW`는
  이 저장소의 `main`을 clone한 것이므로, 이 저장소에서 커밋·푸시한
  변경을 `C:\CNW`에 실제로 적용하려면 그 디렉터리에서 `git pull` 후
  필요시 `docker compose up -d --build`로 재기동해야 한다 - 이 저장소의
  소스만 고치고 `C:\CNW`가 자동으로 최신을 반영한다고 가정하지 않는다.
  반대로 `C:\CNW`에서 직접 코드를 고치는 일은 없어야 한다(그건 이
  저장소의 역할).
- **설치/업그레이드 절차를 검증할 땐 `C:\CNW`가 유일한 실측 대상**:
  README.md의 설치·업그레이드·마이그레이션 안내가 실제로 맞는지
  확인하려면(예: 새 CLAUDE.md 절 추가, 새 마이그레이션 스크립트 등)
  이 저장소 안에서 임시 스택을 새로 만들지 말고 `C:\CNW`에서 그
  절차를 그대로 실행해 검증하는 걸 우선 고려한다 - 실제로 "완전히 새로
  설치"하는 경로의 버그(`.gitattributes` CRLF, Gitea `INSTALL_LOCK`)는
  이미 오래 떠 있던 dev 스택으로는 발견되지 않고 `C:\CNW`를 처음
  구성하면서만 드러났다.
