...
요약 표/절에 직접 영향이 가는 변경이면 더 우선한다). **새 CLI/MCP
명령을 만들 때는 `backend/src/core/monitoringRegistry.ts`(사용
모니터링의 명령 이름 매핑, §28)도 같은 라운드에 같이 갱신한다** -
안 해도 기능은 안 죽지만(raw route로 표시), 모니터링 대시보드가
명령 이름으로 안 보이게 된다.
