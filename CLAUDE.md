...
**패키지 구조/구현 현황의 자세한 서술은 [docs/project-structure.md](docs/project-structure.md)
에 있다** - CLAUDE.md는 매 세션이 읽는 온보딩 문서라 여기서는 요약만
유지하고, 새 라운드마다 기능이 실제로 동작하는 걸 확인하면 그 문서를
갱신한다(이 문서가 아니라). 진행 중인 작업은 `docs/plan-*.md`(문서
엔티티 모델과 동일한 frontmatter, `state`로 진행 상태 추적)로
남기고 순서대로 처리한다 - `docs/index.md`가 전체 목차다.

5개의 독립 패키지: `backend/`(Express + Prisma, REST는
`api/rest.ts`/CLI·MCP는 `POST /api/actions`), `shared/`(CLI/MCP
공용 HTTP 클라이언트+설정 해석, backend와 무관), `cli/`, `mcp/`,
`frontend/`(Quasar+Vue3+TS). 루트 `package.json`은 npm workspaces
선언만 있는 최소 파일.
...
1. **계획을 문서로 남긴다** - 여러 단계로 나뉘는 작업을 시작하기 전에
   `docs/plan-<slug>.md`를 만든다(문서 엔티티 model과 동일한 frontmatter
   - `type: plan`, `kind: PL`, `state`는 `added`(대기)/`active`(진행
   중)/`done`(완료)로 그 라운드 안에서 직접 갱신). 여러 계획이 쌓이면
   `docs/index.md`에 나열된 순서대로 **하나씩 순차적으로** 처리한다 -
   동시에 여러 계획을 벌리지 않는다.
2. **조사** - 비슷한 걸 `v2`/`concept`에서 이미 겪었는지, 또는 이미
   `docs/`에 관련 기록이 있는지 먼저 확인한다(그대로 가져다 쓰지는
   않는다).
3. **구현**.
4. **검증** - 실제로 기동해서 왕복 확인한 뒤에만 "완료"로 본다.
5. **문서화** - 그 계획 문서의 진행 상태(체크박스/`state`)를 갱신하고,
   완료된 기능은 [docs/project-structure.md](docs/project-structure.md)
   에 반영한다.
6. **기록 연결** - 위 일곱 가지 규칙대로 코드 관계/연관 소스/브랜치/
   칸반/계획/문서 링크를 실제로 남긴다.
7. **커밋/푸시/배포** - 설계자가 명시적으로 요청했을 때만 한다.
