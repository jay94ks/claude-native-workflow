...
- **이 저장소 자신을 탐색하며 발견한 코드 관계도 실제로 기록한다** -
  이 저장소가 연동된 프로젝트는 코드 관계도(`#code-relation-graph`)
  기능을 그대로 쓸 수 있는 실제 git 저장소이기도 하다. 여러 파일을
  가로지르는 탐색이라 다시 파악하려면 비용이 드는 발견을 했으면(예:
  "이 버그가 세 파일에 걸쳐 있다는 걸 확인했다") `docs relation add`로
  기록하고, 새로 탐색을 시작하기 전엔 `docs relation list --q
  <키워드>`로 이미 기록된 게 있는지 먼저 확인한다(SKILL.md "코드
  관계도" 절의 기준을 그대로 따름 - 사소한 한 줄짜리 조회까지 전부
  남기는 감사 로그가 아니다). 현재 체크아웃된 브랜치가 자동 감지되므로
  별도 설정이 필요 없다.
