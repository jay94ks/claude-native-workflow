...
4. **연관 소스/브랜치/칸반 카드도 실제로 연결·기록한다**: `DN` 라운드
   ...
   브랜치에서 작업했다면 `docs link-branch`도, 이 저장소처럼 `main`에
   직접 커밋하는 흐름이면 생략) - 라운드를 끝내며 FT/DN을 갱신하는
   시점에 같이 하는 루틴으로 삼는다. 프로젝트에 칸반 컬럼이 있으면
   (`docs kanban-columns`) 완료한 작업 단위마다 관련 문서를 `--refs`로
   묶어 카드를 만들거나(`docs kanban-card-new`) 진행 상황에 맞게
   옮긴다(`docs kanban-card-move`) - 코드 관계도와 같은 기준으로, 사소한
...
6. **문서 검색은 `--codes-only`를 기본으로 쓴다**: `docs search`는
   ...
   그대로 출력에 남기지 않는다.
7. **CLAUDE.md/SKILL.md(+ 배포 템플릿)도 실제로 동기화한다**: 새
   기능을 추가하거나 기존 CLI/MCP 동작(새 옵션, 응답 형태 변화, 새
...
4. **문서화** - `FT` 문서(그 기능이 지금 무엇을 하는지만, 설계
   ...
   두면 안 된다). 새 기능이거나 CLI/MCP 동작이 바뀌었다면 규칙 7대로
   CLAUDE.md/SKILL.md(+ 배포 템플릿)도 같이 갱신한다.
5. **기록 연결** - 이번 라운드에서 실제로 발견한 코드 관계(이
   ...
   생략)로 그 문서에 연결한다. 프로젝트에 칸반 컬럼이 있으면(`docs
   kanban-columns`) 완료한 작업 단위마다 관련 문서를 `--refs`로 묶어
   카드를 만들거나(`docs kanban-card-new`) 진행 상황에 맞게
   옮긴다(`docs kanban-card-move`). 지금 처리하지 않고 나중에 따로
   계획을 잡아야 할 항목을 발견했다면 규칙 5대로 `docs plan new`로
   남겨둔다.
