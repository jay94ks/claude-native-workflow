...
`docs/PROTOCOL.md` 7절과 동일 — 미답변 질문과 확인 안 된 변경을 먼저
알려준다:

```bash
docs3 pending <projectId>          # 미답변 (Qn) 목록
docs3 changes list <projectId>     # 내가 없는 사이(다른 설계자 git push 등) 바뀐 것
```
...
docs3 design <projectId>                  # DC/RV/FX 목록
docs3 logs <projectId>                    # LG 목록
docs3 all <projectId>                     # 전체 문서 목록
docs3 search <projectId> <검색어>          # 전문 검색
docs3 validate <projectId>                # docs/ 구조 검증
```
...
docs3 git log <projectId> [--path <path>] [--limit <n>]   # 커밋 이력(읽기 전용)
docs3 git show <projectId> <sha>                            # 커밋 1건 상세
docs3 git diff <projectId> <sha>                             # 커밋 diff
docs3 git blame <projectId> <path>                           # 줄별 이력
```
...
```bash
docs3 comment list <projectId> <path>
docs3 comment add <projectId> <path> <내용...>
docs3 comment resolve <projectId> <path> <commentId>
```

`docs/PROTOCOL.md`의 공식 답변 대기(`DC`/`RV`/`FX`의 `RP`)와는 별개인,
문서 단위의 가벼운 토론 스레드다. 프로젝트마다 하나의 공유 서비스 DB에
`project_id`로 나뉘어 저장된다(SP-00002 8절).

## 변경 추적 큐

```bash
docs3 changes list <projectId>
docs3 changes ack <projectId> <id>
```

확인한 항목은 `ack`로 큐에서 지운다.
...
docs3 member-role <projectId> <userId> --role viewer|editor|owner
docs3 member-remove <projectId> <userId>
```
...
| 명령 | 설명 |
|---|---|
| `docs3 register --api <url> --username <u> --email <e> [--password <p>]` | 계정 생성 |
...
| `docs3 member-role <projectId> <userId> --role <r>` | 멤버 역할 변경(owner만) |
| `docs3 member-remove <projectId> <userId>` | 멤버 제거(owner만) |
| `docs3 tree <projectId>` | 문서 트리 |
| `docs3 doc <projectId> <path>` | 문서 1건 조회 |
| `docs3 pending <projectId>` | 답변 대기 목록 |
| `docs3 design <projectId>` / `logs <projectId>` / `all <projectId>` | 타입별 목록 |
| `docs3 search <projectId> <검색어>` | 전문 검색 |
| `docs3 validate <projectId>` | 구조 검증 |
| `docs3 new <projectId> <type> --title <t> [--links <ids>]` | 새 문서 생성 |
| `docs3 save <projectId> <path> <file>` | 문서 본문 전체 갱신(본문만, 프론트매터 제외) |
| `docs3 reply <projectId> <path> <qid> <답변...>` | 답변 대기 질문에 답변 |
| `docs3 transition-done <projectId> <planId> --report <r>` | PL → DN 전환 |
| `docs3 git commit <projectId> [-m <msg>]` / `push` / `pull` | git 동기화 |
| `docs3 git log <projectId> [--path] [--limit]` / `show <sha>` / `diff <sha>` / `blame <path>` | git 이력(읽기 전용) |
| `docs3 comment list/add/resolve <projectId> <path> ...` | 문서 코멘트 |
| `docs3 changes list <projectId>` / `changes ack <projectId> <id>` | 변경 추적 큐 |
