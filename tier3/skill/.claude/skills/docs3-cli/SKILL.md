...
로컬 파일(프론트매터 포함 전체 마크다운)의 내용을 그대로 문서 본문에 덮어쓴다.
절차: `docs3 doc`으로 현재 내용을 받아 → 로컬에 임시 파일로 저장 → 필요한
부분만 고쳐 씀(프론트매터의 `id`/`type`은 유지, `updated` 갱신, `docs/PROTOCOL.md`
4절 형식으로 "## 답변 대기" 항목 추가 등) → `docs3 save`로 반영. `DC`/`RV`/`FX`
문서에 답변 대기 질문을 추가하는 것도 이 명령으로 한다(질문 추가 자체는
서버가 자동화해주지 않음 — 4절 형식을 직접 지켜서 써야 함).
