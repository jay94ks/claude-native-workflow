...
**중요 — `docs/reply/index.md`는 자동으로 갱신되지 않는다.** 이 표는
`tools/docs` 대시보드의 답변 처리 흐름(6절, `answer_pending()`)에서만
재생성된다. Claude가 대화 중 대시보드를 거치지 않고 `docs/*.md`를 직접 써서
새 "## 답변 대기" 섹션을 추가하거나 기존 섹션을 고칠 때는 이 표가 갱신되지
않으므로, **그 직후 반드시**
`python tools/docs/server.py --rebuild-reply-index`를 실행해 동기화한다
(서버가 이미 떠 있어도 상관없이 별도 프로세스로 즉시 끝난다 - 서버 기동
없이 종료). 이걸 빠뜨리면 방금 만든 질문이 `docs/reply/index.md`에서 계속
누락된 채로 남아, 7절 5번("대화 시작 시 미답변 항목 알림")이 조용히
틀린 정보를 줄 수 있다.
