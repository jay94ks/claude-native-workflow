...
붙은 기능 브랜치에서 작업했다면(이 저장소 자신처럼 `main`에 직접
커밋하는 흐름이면 의미가 없으니 생략) 그 브랜치명을 관련 문서에
남겨, 나중에 "이 문서가 어느 브랜치들을 거쳐 구현됐는지" 역사적으로
추적할 수 있게 한다.
...
| 두 지점 사이 range diff(커밋 1건이 아니라 base..head 전체) | `docs git compare <projectId> <base> <head>` | `git_compare` |
...
`docs`)는 이 프로젝트의 전체 문서 목록(+각자의 `updatedAt`)을 DB에서
다시 받지만, **실제로 파일을 다시 쓰는 건 `updatedAt`이 지난 동기화
때와 달라진(또는 파일이 아예 없어진) 문서로만 좁힌다** - 판단 기준은
`dir/index.md` 표의 "최종 수정" 열(다음 sync가 그 값을 읽어 대조,
별도 상태 파일 없이 이 표 자체가 지난 동기화 결과를 겸함). 안 바뀐
문서는 디스크에 아예 손대지 않는다 - 이 캐시를 git으로 커밋해 쓰는
프로젝트라면 매번 전체 파일이 diff에 잡히는 낭비가 없다는 뜻. 새로
쓰는 각 `dir/<trackingCode>.md`는 제목 헤딩 + "손으로 편집하지
마세요"/트래킹코드/상태/`updatedAt`을 알리는 HTML 주석 + 본문,
`dir/index.md`(추적코드-제목-상태-최종수정 표)는 매번 다시 만든다 -
**정본은 항상 DB**, 이 캐시 파일들은 읽기 전용 사본이라 직접 고치지
않는다. 삭제됐거나 다른 프로젝트로 옮겨진 문서의 캐시 파일은 이번에
못 받았으면 자동으로 정리된다(`dir` 안의 `XX-XXXXXXXX.md` 패턴 파일
목록으로 판단). `docs cache clean [dir]`(MCP `cache_clean`)은 `dir`
안에서 `sync`가 만든 걸로 알아볼 수 있는 파일(그 패턴 + `index.md`)만
지운다 - 디렉터리 자체나 무관한 파일은 안 건드린다.
