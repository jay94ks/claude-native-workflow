...
- `bootstrap-prompt.md` — 실제 배포용 산출물(초간단 등급). 이 파일 전체를
  복사-붙여넣기한다.
- `tier1/` — 위 프롬프트에 실제로 삽입되는 파일들의 **참조 원본**(pristine,
  빈 상태)이자 로컬 개발/테스트용 실행 가능한 사본. 여기서 대시보드를 직접
...
- `scripts/build_prompt.py` — `tier1/` 아래 참조 원본 파일들을 읽어
  `bootstrap-prompt.md`를 재조립하는 개발용 스크립트. `tier1/`을 수정했으면
  반드시 다시 실행해서 프롬프트를 최신 상태로 맞춘다.
...
`bootstrap-prompt.md`가 `tier1/CLAUDE.md`/`tier1/docs/`/`tier1/tools/docs/`
내용 그대로 재생성된다. 수동으로 `bootstrap-prompt.md`를 직접 편집하지
않는다(다음 재생성 때 덮어써진다). **`tier1/`은 항상 빈 템플릿 상태를
유지해야 한다** — 이 저장소 자신의 `docs/`(DS-00001 등 실제 문서)가 실수로
`tier1/`에 섞여 들어가면 새 프로젝트에 엉뚱한 내용이 복붙되므로 주의.
