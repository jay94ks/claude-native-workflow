...
**아래 절차를 손으로 직접 따라갈 필요는 없다** - 연동하려는 그 작업
폴더에서 Claude Code(또는 이 시스템과 연동해 쓰는 다른 AI 코딩 에이전트)
세션을 열고, 이 저장소 README 링크와 현재 설치 상태만 알려주면 나머지는
AI가 아래 시나리오 표를 스스로 읽고 판단해 진행한다. 예:

> 이 폴더에서 워크플로우 관리자를 사용하려고 해.
> 해당 관리자의 리포는 "https://github.com/jay94ks/claude-native-workflow/blob/main/README.md"이고,
> 현재 로컬 C드라이브의 CNW 디렉터리에 설치가 완료되어 있으므로 이
> 폴더를 위한 프로젝트와 리포를 준비하여 연동작업을 할 계획을 세워.

이 한 문장에 "설치 여부"(이미 있음)와 "위치"(로컬 `C:\CNW`)가 들어있어
AI가 시나리오 2("로컬 설치(기존) + 현재 작업 폴더 연동")로 바로 좁혀
계획을 세울 수 있다 - 설치가 없거나 위치가 다르면 그만큼만 문장을
바꿔 알려주면 된다(예: "원격 서버 https://... 에 설치돼 있어" →
시나리오 5). 이 폴더에 이미 `CLAUDE.md`/기존 문서가 있으면 그 내용도
같이 훑어 마이그레이션이 필요한지까지 함께 판단한다.
