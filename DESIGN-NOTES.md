...
## 사용 모니터링에 평균 소요 시간(avgMs) 추가

설계자 요청("사용 모니터링에서 각 호출에 소요된 평균 시간까지
반영해줘")에 따라 앞선 사용 모니터링 라운드(바로 위 절, FT-728E3CD4/
DN-A9E5B1C6)에 호출당 평균 소요 시간을 추가했다.

**구현**: `RequestStat`에 `totalDurationMs Int @default(0)` 컬럼
추가(스키마 3종) - 별도의 이동평균 컬럼 없이 조회 시점에
`totalDurationMs / count`(반올림)로 매번 계산(`avgMs`). 전역
모니터링 미들웨어가 요청 진입 시각을 재서 `res.on("finish")`
시점까지의 경과를 `recordRequest()`에 넘긴다. 설치 전체(합산) 조회는
여러 프로젝트의 같은 (method, routePattern, origin) 행을 합칠 때
`totalDurationMs`도 먼저 다 더한 뒤 마지막에 한 번만 나눠 계산한다
(중간에 평균의 평균을 내면 왜곡되므로). 웹 "모니터링" 탭의 명령
사용 빈도 표에 "평균 소요 시간" 열 추가. CLI/MCP는 응답 필드만
늘어나 명령 표면 자체는 안 바뀜(SKILL.md 갱신 불필요).

**검증**: `C:\CNW-test`에서 `CNW_SESSION_NAME` 고정 후 `docs list`/
`docs plan list`를 여러 번 호출해 `docs monitoring stats`로 avgMs가
실측치(수~수십 ms)로 합리적으로 나오는지 확인, 연이은 패턴 집계도
기존처럼 정상 동작함을 재확인. 웹 "모니터링" 탭에서도 새 열이 실제
숫자로 렌더링되고 이 기능과 관련된 콘솔 에러가 없음을 확인.
