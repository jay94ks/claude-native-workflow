...
### 2026-09-10 (계속 33) — RV-00001 결합안 1번: 빈 폴더 숨김 + 개수 배지 + 주의 배지

설계자가 [RV-00001](docs/review/RV-00001.md)에 "결합안 1번부터"로
답변해 착수. 문서 없는 타입 폴더는 트리에서 숨기고, 있는 폴더는
`spec (4)`처럼 개수 배지를 달고, 답변 대기/미확인 변경이 있는 문서
행엔 주의 아이콘을 붙였다 - tier2/dashboard의 `DocTree.vue`(tier3가
재사용)와 tier1의 vanilla JS `app.js` 양쪽에 대칭으로 구현.

만드는 과정에서 실제 버그 두 개를 더 찾았다: (1) 프론트매터
`reply_pending`이 답변 처리 경로에서만 갱신되고 새 질문을 막 적어
넣을 땐 전혀 갱신 안 됨 - 트리가 신뢰하는 값을 프론트매터 대신
본문을 직접 스캔한 결과로 바꿔서 고침(tier1/tier2 둘 다). (2) 그
수정 때문에 PROTOCOL.md 본문에 예시로 들어있는 "- [ ] (Q1) ..."
텍스트가 진짜 질문으로 오인돼 PROTOCOL.md도 답변 대기로 잘못 뜨는 걸
tier1 대시보드에서 실제로 발견 - `isDocFile()` 조건을 배지에도 걸어
막음. RV-00001 자신의 질문도 형식 실수(태그 없음+줄바꿈)로 못 읽히고
있던 걸 고치고, 이번 메시지로 실제 답이 나왔으니 `docs reply`로 정식
처리(RP-00011).

이 저장소 자신의 실제 docs/로 tier1(8756)/tier2 대시보드+백엔드
(9200/8766, 둘 다 읽기 전용으로 이 저장소를 가리킴)를 실제로 띄워
스크린샷까지 확인 - 빈 폴더 사라짐, 개수 배지 정확, DC-00003에만
주의 아이콘, 콘솔 에러 없음. `docs/reply/index.md` 표 자체가 같은
